...
function applyRoleNav() {
  // 仅 admin（于荣华）可在质检/作业之间切换；member 账号只留在作业，qc 只留在质检
  const auth = getAuth();
  const role = (auth.user && auth.user.role) || '';
  const link = $('linkQc');
  if (link && role !== 'admin') link.classList.add('hidden');
}
...
function renderAnnotatorSelect(annotators) {
  ...
  // 非 admin 只有一个作业员（当前用户）时，隐藏「作业员」下拉
  const field = sel.closest('.anno-field');
  if (field) field.classList.toggle('hidden', annotators.length <= 1);
