...
# 登录 token 与最近提交均不再落盘（token 存浏览器 localStorage，最近提交也存浏览器），
# 因此 exe 同目录不会产生任何多余文件。
...
# 内嵌管理员账号：平台「保存标注」/save 只允许 admin/作业员，qc 角色无权；
# 质检员在质检平台修改标注保存时改用 owner token（质检提交 /qc 仍用质检员本人 token）。
_OWNER_EMAIL = "v_yuronghua@baidu.com"
_OWNER_PASSWORD = "pw123456"
_OWNER_TOKEN = None

# 登录会话：token -> user（含 role）。_route 里 USER 会被改写成 {id}，这里保留完整信息。
_LOGIN_SESSIONS = {}


def _get_owner_token():
    """登录 owner（管理员）并缓存 token；失败返回 None（调用方回退当前登录 token）。"""
    global _OWNER_TOKEN
    if _OWNER_TOKEN:
        return _OWNER_TOKEN
...
def _owner_name_map():
    """uid -> 名字，用 owner token 拉 /api/admin/monitoring（member 无权限访问 admin 接口）。"""
    m = {}
    tok = _get_owner_token()
    if not tok:
        return m
    s, h, raw = upstream("GET", "/api/admin/monitoring", token=tok)
    try:
        mon = json.loads(raw.decode("utf-8"))
    except Exception:
        return m
    for p in mon.get("projects", []):
        for a in (p.get("qc_assignees") or []):
...
def fetch_annotations(pid, image_ids, max_workers=SORT_CONCURRENCY):
    """并发拉取每张图的实时 qc_status + reviewed_at（每次现查，不用缓存）"""
    data = {}
    ...
    def _one(img):
        ...
        try:
            d = json.loads(raw.decode("utf-8"))
            return img, {"qc_status": d.get("qc_status") or "",
                         "reviewed_at": d.get("reviewed_at") or ""}
        except Exception:
            return img, {"qc_status": "", "reviewed_at": ""}
...
# ======================================================================
def _current_uid():
    """当前登录用户 id（_route 已从 X-User-Id 头写入 USER）。"""
    return (USER or {}).get("id") or ""


def qc_setup():
    """返回当前登录用户有质检权限的项目列表（按分配表判断，不用缓存的 is_qc）"""
    uid = _current_uid()
...
def qc_save(pid, image_id, boxes):
    ...
    # 平台 /save 只允许 admin/作业员，qc 角色无权；qc 角色改用 owner token 保存，
    # admin/member 继续用本人 token（保证作业员保存的归属不受影响）。
    role = (USER or {}).get("role") or (_LOGIN_SESSIONS.get(TOKEN) or {}).get("role") or ""
    tok = _get_owner_token() if role == "qc" else None
    s, h, raw = upstream("POST", "/api/projects/%s/save" % pid,
                         body={"image_id": image_id, "boxes": boxes}, token=tok)
    try:
        d = json.loads(raw.decode("utf-8"))
    except Exception:
        d = {}
...
def qc_assigned(pid, uid, status, offset, limit):
    """返回某质检员在指定 qc_status 下的图片列表（分页）"""
    proj = get_project(pid)
    if not proj:
        raise RuntimeError("项目不存在：" + pid)
    files = (proj.get("qc_assignments") or {}).get(uid) or []
...
def anno_setup():
    """返回当前登录用户有作业权限的项目列表（按分配表判断，不用缓存的 is_assigned）"""
    uid = _current_uid()
    proj_data = get_projects()
    out = []
    for p in proj_data.get("projects", []):
        if uid not in (p.get("assignees") or []) and uid not in (p.get("assignments") or {}):
            continue
        out.append({"id": p.get("id"), "name": p.get("name"),
                    "categories": p.get("categories") or []})
    return out
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _route(self, method):
        ...
        if path == "/api/me" and method == "GET":
            ...
            return self._send_json({"ok": False, "need_login": True}, 401)
        ...
        if path == "/api/my_daily_stats" and method == "GET":
            gate = self._need_login()
            if gate:
                return gate
        ...
        if m and method == "GET":
            ...
            if gate:
                return gate
            try:
                return self._send_json({"ok": True, **qc_assigned(pid, uid, status, offset, limit)})
            except RuntimeError as e:
                return self._send_json({"ok": False, "error": str(e)}, 400)
